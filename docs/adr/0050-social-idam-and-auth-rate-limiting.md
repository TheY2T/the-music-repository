...
- **Decision:**
  - **Social providers (Google, Facebook, Apple).** Configured under `socialProviders` in
    ...
    Apple's client secret is a short-lived **ES256 JWT** signed from the `.p8` key with `jose`
    (`generateAppleClientSecret`, its own module for testability), so Apple uses the async-factory
    provider form; `https://appleid.apple.com` is auto-added to `trustedOrigins` when Apple is
    configured (it POSTs the callback as `form_post`).
  - **Account linking.** `account.accountLinking.enabled` links a social sign-in to an existing account
    when the provider reports the email as verified. `trustedProviders: ['google', 'apple']` additionally
    links those two without a verification check (both reliably verify email); Facebook is left to the
  ...
  - **Provider brand marks.** Google/Facebook/Apple logos aren't in the Lucide set and must keep each
...
- **Consequences:**
  - Provider credentials and `AUTH_RATE_LIMIT_ENABLED` are env-driven: secrets live in Render's `dev`
    ...
    callback URI registered; OAuth round-trips can't be completed locally without one.
  ...
  - Apple's `.p8` key may be stored with literal `\n` escapes (single-line env value); the JWT signer
    restores real newlines before parsing.
